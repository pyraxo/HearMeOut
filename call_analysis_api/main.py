...
def match(request: CallData):
    ...
    for i in range(max_calls):
        try:
            ...
            representativeObject = RepresentativeScore(representative_id=request.representative_id,bandwidth_change=representative_score)
            payload = {
                "representative_id": request.representative_id,
                "bandwidth_change": representative_score
            }
            response = requests.post("http://127.0.0.1:8001/updatebandwidth",json=payload)
