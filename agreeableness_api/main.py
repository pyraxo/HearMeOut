...
def aggregate_expressions(expressions):
    ...
    for total_score in total_expression_scores:  # getting the average expression score
        total_expression_scores[total_score] = total_expression_scores[total_score] / \
            expressions_count
...
def get_customer_score(average_expression_scores):
    ...
    positive_expression_points = joy * 0.7 + love * \
        0.7 + satisfaction * 0.5 + calmness * 0.1
    negative_expression_points = anger * 0.07 + disappointment * 0.07 + \
        disgust * 0.05 + distress * 0.1 + horror * 0.1 + pain * 0.8 + sadness * 0.8
